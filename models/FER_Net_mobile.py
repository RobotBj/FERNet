...
class BasicFER(nn.Module):

    def __init__(self, in_planes, out_planes, stride=1, scale=0.1):
        super(BasicFER, self).__init__()
        self.scale = scale
        self.out_channels = out_planes
...
class BasicFER_PRO(nn.Module):

    def __init__(self, in_planes, out_planes, stride=1, scale=0.1):
        super(BasicFER_PRO, self).__init__()
        self.scale = scale
        self.out_channels = out_planes
...
class FERNet(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes):
        ...
        if size == 300:
            ...
            print("Error: Sorry only FER300_mobile is supported!")
            return

        self.base = nn.ModuleList(base)
        self.Norm = BasicFER_PRO(512, 512, stride=1, scale=1.0)
...
def add_extras(size, cfg, i, batch_norm=False):
    ...
    for k, v in enumerate(cfg):
        if in_channels != 'S':
            if v == 'S':
                layers += [BasicFER(in_channels, cfg[k+1], stride=2, scale=1.0)]
            else:
                layers += [BasicFER(in_channels, v, scale=1.0)]
        in_channels = v
    if size ==300:
...
def build_net(phase, size=300, num_classes=21):
    ...
    return FERNet(phase, size, *multibox(size, MobileNet(),
