...
class VOCDetection(data.Dataset):
    ...
    def __init__(self, root, image_sets, preproc=None, target_transform=None,
        ...
        # self._annopath = os.path.join('%s', 'test_box', '%s.xml')
        # self._imgpath = os.path.join('%s', 'test_images', '%s.jpg')
        # self._annopath = os.path.join('%s', 'total_box', '%s.xml')
        # self._imgpath = os.path.join('%s', 'total_images', '%s.jpg')
...
#total train data
        # rootpath = os.path.join(self.root)
        # for line in open(os.path.join(rootpath, 'total_train_data.txt')):
        #     self.ids.append((rootpath, line.strip()))
#test
        # rootpath = os.path.join(self.root)
    ...
    def _get_voc_results_file_template(self):
        filename = 'nonlocal planb loss theresh = 0.3.txt' #+ '_{:s}.txt'
        filedir = os.path.join(
            self.root, 'results', 'Main', 'nonlocal planb')
        if not os.path.exists(filedir):
            os.makedirs(filedir)
        path = os.path.join(filedir, filename)
        return path
