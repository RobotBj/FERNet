...
class FBResNet(nn.Module):
    ...
    def forward(self, input):
        x, s0, s1, s2, s3 = self.features(input)
        x = self.logits(x)
        return x, s0, s1, s2, s3


def fbresnet18(num_classes=1000,pretrained=True):
...
def RFBassistantnet(num_classes=4):
    net1 = fbresnet18(num_classes)

    x, x0, x1, x2, x3 = net1(input)
...
if __name__ == '__main__':
    net = fbresnet18(4,pretrained=True)
    print(net)
    img_path = '/home/ch/data/train_data/test_images/G0024174_1142.jpg'
    img = cv2.imread(img_path)
    transform = BaseTransform(300, (64.238223, 142.588190, 56.799253), (2, 0, 1))
    with torch.no_grad():
